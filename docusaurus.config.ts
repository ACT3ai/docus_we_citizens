...
const config: Config = {
  ...
  plugins: [
    [
      ...
      {
        // Docs moved from /docs/* to the site root on 2026-09-17. Links already
        // out in the world (X posts, search results, the web app) still point at
        // /docs/..., so every doc page also gets a redirect page at its old
        // /docs/ address. GitHub Pages cannot send HTTP 301s, so these are HTML
        // pages carrying a meta refresh and a canonical link to the new address.
  ...
  presets: [
    [
      'classic',
      {
        docs: {
          path: 'site/docs',
          // Serve docs at the site root: site/docs/influencers/ is published at
          // /influencers/, not /docs/influencers/. The files stay in site/docs/.
          // Every old /docs/... address still works — see the client-redirects
          // plugin below.
          routeBasePath: '/',
