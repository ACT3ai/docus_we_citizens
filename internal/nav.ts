...
const SUB_LABELS: Record<string, string> = {
  ...
  congress_for_citizens: "Who Congress actually works for",
  non_partisan_site: "Why this door takes no side",
  ...
  american_ethics: "What capture costs beyond money",
  problems: "The ranked, sourced catalog",
  rigged_economy: "Named out loud, with receipts",
  deep_state: "Documented, sourced at the door",
  us_intelligence: "Serving this country only",
  foreign_intelligence: "Nobody else picks our leaders",
  influencers: "Who pays the voice in your feed",
  election_integrity: "Check the count yourself",
  no_forever_wars: "No needless war, no draft",
  no_social_credit: "The American version, named",
  ...
  replace_incumbents: "Elections, and nothing else",
  ...
  trust_scores: "Real citizens, counted as real",
 ...
 *
 * FOUR DOORS, FOUR AUDIENCES. R is Republicans, D is Democrats, L is
 * Libertarians, S is Democratic Socialists. That is the whole mapping, and it is
 * the only thing the four entries below have in common.
 *
 * THERE IS NO MIRROR RULE AND NO PAIRING. The doors were paired once — R with D
 * on a "partisan axis", L with S on an "economic axis" — and every door was
 * required to be its partner's reflection with the label swapped. That rule was
 * REMOVED on 2026-08-21. No door is another door's mirror, no door needs a
 * partner to exist, and a change to one door creates no obligation to any other.
 * Each edition's content is decided by its own marketing/messaging file
 * (republicans.md, democrats.md, libertarians.md, socialism.md) and by nothing
 * else.
 *
 * The four entries live here rather than being read out of level_2.csv's
 * ordering because they are pinned top-level navbar items, not part of the
 * ordered reading path. Their rows are still in level_2.csv so the registry
 * stays complete, and the loop above fails the build if any of the four is
 * missing from it.
 */
 ...
 *
 * The bar carries R and D. L and S were taken off it on 2026-08-17: four party
 * buttons plus the site links plus the app CTA left the row with nothing to
 * give, and the two smaller doors were the ones paying for it. They are not
 * demoted — both are still full Level 2 areas, both are listed under "Party
 * front doors" in the "More" menu and in the footer, and their overview pages
 * hand the visitor WeCitizensL.com / WeCitizensSocialism.com the same way the
 * bar button did.
 *
 * This list is a WIDTH decision, nothing more. It used to be constrained to
 * "only ever a whole pair" by the mirror rule; that rule is gone, so any subset
 * of the four may sit on the bar. Re-check the width budget in
 * internal/css/custom.css ("Fitting the bar") before adding one.
 */
 ...
 * internal/sidebars.ts so that adding a door cannot forget to add its bar. That
 * is wiring, not symmetry — each bar is autogenerated from its own directory
 * tree, so the four doors carry whatever areas they carry and are expected to
 * differ.
 *
...
/**
 * Sidebar id for a party edition. One of the THREE PLACES that must agree for a
 * front door to build at all — the other two being the directory name under
 * site/docs/ and the `level_2_key` column in level_2.csv.
 */
